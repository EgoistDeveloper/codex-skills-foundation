...
The published beta.2 artifact set uses `scripts/release_candidate.py` as the single deterministic identity owner. Its manifest contains no timestamps or local paths: it binds a clean, non-detached Git commit; the prerelease tag; catalog and provider marketplace hashes; exact package names, versions, filenames, byte sizes, SHA-256 values, archive-derived content hashes, and skill counts; plus the exact `SHA256SUMS` hash. The checksum graph is acyclic because package ZIPs are hashed by `SHA256SUMS`, while the separately distributed candidate manifest records the checksum-file hash and is not listed inside `SHA256SUMS`.
...
The `v0.3.0-beta.2` release is bound to commit `98658cd359a05022247622ae00e805ada6c7cfbd`, candidate-manifest SHA-256 `a22be1e252142da8abcab84a0f18006319245702f5f8f03c872c3c85d101ddcc`, and Codex CLI `0.148.0-alpha.15`. The final exact-artifact campaign `20260819-222102-cb0b3908` passed the five-package/fourteen-skill lifecycle, three positive and three negative repetitions, bounded read-only delegation, failed-evidence refusal, all scorer gates, and complete state restoration in exactly sixteen authenticated turns. The release record is [`releases/v0.3.0-beta.2.md`](releases/v0.3.0-beta.2.md).
